...
class PlumberTest(unittest.TestCase):
    ...
    def test_simple_pipeline(self):
        ...
        pipeline1 = builder1.source({"endpoint": Timer, "period": 1.0}).to({"endpoint": MessageModifier}).process(lambda ex: print(ex.in_msg.body + " Pipeline1"))
        pipeline2 = builder2.source({"endpoint": Timer, "period": 2.0}).to({"endpoint": MessageModifier}).process(lambda ex: print(ex.in_msg.body + " Pipeline2"))
